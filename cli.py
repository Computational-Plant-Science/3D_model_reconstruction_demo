...
def reconstruct(source, output_directory, preprocess, gpu):
    ...
    subprocess.run(f"colmap feature_extractor --image_path {source} --database_path {database}" + ('' if gpu else ' --SiftExtraction.use_gpu=0 --SiftExtraction.num_threads=2 --SiftExtraction.first_octave 0'), shell=True)
